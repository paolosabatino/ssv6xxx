			...
			list_for_each_entry(ta, &sc->encrypt_task_head, list) {
				if (cpu == hotcpu) {
					...
					if ((ta->started == 0)
						...
						ta->started = 1;
						printk
						    ("wake up encrypt_task %p state is %ld, cpu = %d\n",
						     ta->encrypt_task,
...
static int ssv6xxx_init_softc(struct ssv_softc *sc)
{
	...
	for_each_cpu(cpu, cpu_present_mask) {
		...
		ta->encrypt_task =
		    kthread_create_on_node(ssv6xxx_encrypt_task, sc,
					   cpu_to_node(cpu),
					   "%d/ssv6xxx_encrypt_task", cpu);
		init_waitqueue_head(&ta->encrypt_wait_q);
		if (!IS_ERR(ta->encrypt_task)) {
			printk
			    ("[MT-ENCRYPT]: create kthread %p for CPU %d, ret = %d\n",
...
void ssv6xxx_deinit_prepare(void)
{
	...
	ssv6xxx_watchdog_controller(ssv_notify_sc->sh,
...
static int ssv6xxx_deinit_softc(struct ssv_softc *sc)
{
	...
	if (!list_empty(&sc->encrypt_task_head)) {
		for (qtask =
		     list_entry((&sc->encrypt_task_head)->next, typeof(*qtask),
				list); !list_empty(&sc->encrypt_task_head);
		     qtask =
		     list_entry((&sc->encrypt_task_head)->next, typeof(*qtask),
				list)) {
			counter++;
	...
	*((u32 *) & chip_id[12]) = (u32) LONGSWAP(regval);
	...
	SMAC_REG_WRITE(sh, ADR_RX_FLOW_DATA,
		       M_ENG_MACRX | (M_ENG_ENCRYPT_SEC << 4) | (M_ENG_HWHCI <<
								 8));
#endif
#if defined(CONFIG_RX_MGMT_CHECK)
