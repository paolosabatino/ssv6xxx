		 ...
		 struct ieee80211_key_conf *key)
{
	...
	*(u16 *) & sram_key->sta_key[0].reserve[0] = key->keylen;
	printk(KERN_ERR
	       "Set WEP %02X %02X %02X %02X %02X %02X %02X %02X... (%d %d)\n",
	...
	if ((sc->sh->cfg.use_wpa2_only == 0)
		...
		if (crypto_data->priv) {
			crypto_data->ops->set_key(key->key, key->keylen, NULL,
						  crypto_data->priv);
			dev_err(sc->dev,
				"[Local Crypto]: VIF gets WEP crypto OK!\n");
			dev_err(sc->dev,
				"[Local Crypto]: Use driver's encrypter.\n");
			vif_priv->need_sw_decrypt = true;
			vif_priv->need_sw_encrypt = true;
			vif_priv->use_mac80211_decrypt = false;
		} else {
			dev_err(sc->dev,
				"[Local Crypto]: Failed to initialize driver's crypto!\n");
			dev_dbg(sc->dev,
				"[Local Crypto]: Use MAC80211's encrypter.\n");
			vif_priv->need_sw_decrypt = false;
			vif_priv->need_sw_encrypt = false;
 ...
 :		    NULL;
		...
		if (temp_crypt_priv) {
			dev_err(sc->dev, "Use driver's %s cipher OK!\n",
				cipher_name);
			temp_crypt->set_key(key->key, key->keylen, NULL,
					    temp_crypt_priv);
			crypto_data->priv = temp_crypt_priv;
			crypto_data->ops = temp_crypt;
		} else {
			dev_err(sc->dev,
				"Failed to initialize driver's %s crypto! "
				    ...
				    struct ieee80211_key_conf *key)
{
 ...
 :		    NULL;
		...
		if (temp_crypt_priv) {
			dev_err(sc->dev,
				"VIF %d gets %s crypto OK! Use driver's crypto.\n",
				vif_priv->vif_idx, cipher_name);
			temp_crypt->set_key(key->key, key->keylen, NULL,
					    temp_crypt_priv);
			crypto_data->priv = temp_crypt_priv;
			...
			vif_priv->use_mac80211_decrypt = true;
			dev_err(sc->dev,
				"VIF %d failed to initialize %s crypto!"
				" Use MAC80211's instead.\n", vif_priv->vif_idx,
				cipher_name);
			ret = -EOPNOTSUPP;
		}
		...
		if (another_vif_priv != NULL) {
			if (((SSV6XXX_USE_SW_DECRYPT(vif_priv)
				...
				if (((val >> 4) & 0xF) != M_ENG_CPU) {
					...
					printk
					    (" doesn't need to change rx flow\n");
				}
			}
		}
	}
...
static int _set_pairwise_key_sms4(struct ssv_softc *sc,
				  struct ssv_vif_priv_data *vif_priv,
				  struct ssv_sta_priv_data *sta_priv,
				  enum SSV_CIPHER_E cipher,
				  struct ieee80211_key_conf *key)
{
	...
	if (crypto_data->priv) {
		...
		dev_err(sc->dev,
			"Failed to initialize driver's SMS4 crypto!\n");
		crypto_data->ops = NULL;
		sta_priv->need_sw_encrypt = false;
		sta_priv->need_sw_decrypt = false;
		ret = -EOPNOTSUPP;
...
static int _set_group_key_sms4(struct ssv_softc *sc,
			       struct ssv_vif_priv_data *vif_priv,
			       struct ssv_sta_priv_data *sta_priv,
			       enum SSV_CIPHER_E cipher,
			       struct ieee80211_key_conf *key)
...
static int _set_key_sms4(struct ssv_softc *sc,
			 struct ssv_vif_priv_data *vif_priv,
			 struct ssv_sta_priv_data *sta_priv,
			 enum SSV_CIPHER_E cipher,
			 struct ieee80211_key_conf *key)
{
	if (key->flags & IEEE80211_KEY_FLAG_PAIRWISE)
		return _set_pairwise_key_sms4(sc, vif_priv, sta_priv, cipher,
					      key);
	else
			   ...
			   struct ieee80211_key_conf *key)
{
	...
	case SET_KEY:
		{
			...
			case SSV_CIPHER_SMS4:
				ret =
				    _set_key_sms4(sc, vif_priv, sta_priv,
						  cipher, key);
				break;
			default:
				break;
			...
			if (sta) {
				...
				if (SSV6200_USE_HW_WSID(sta_idx)) {
					...
					if (sta_priv->has_hw_decrypt) {
						...
						printk
						    ("set hw wsid %d cipher mode to HW cipher for pairwise key\n",
						     sta_idx);
					}
				}
			} else {
				if (vif_info->if_type == NL80211_IFTYPE_STATION) {
					...
					if (SSV6200_USE_HW_WSID
						...
						if (vif_priv->has_hw_decrypt) {
							...
							printk
							    ("set hw wsid %d cipher mode to HW cipher for group  key\n",
							     first_sta_priv->
							     sta_idx);
						}
					}
	...
	case DISABLE_KEY:
		{
			...
			}
			{
				if ((key->keyidx == 0) && (sta_priv != NULL)) {
					unsigned long flags;
					INIT_WRITE_CRYPTO_DATA(crypto_data,
							       &sta_priv->
							       crypto_data);
					sta_priv->has_hw_decrypt = false;
					sta_priv->has_hw_encrypt = false;
					...
					if (crypto_data->ops
						...
						u32 sta_addr4_5 =
						    ...
						    (u16 *) & sta->addr[4];
						u32 removed_skb_num;
						START_WRITE_CRYPTO_DATA
						    (crypto_data);
						crypto_data->ops->
						    deinit(crypto_data->priv);
						crypto_data->priv = NULL;
						crypto_data->ops = NULL;
						END_WRITE_CRYPTO_DATA
						    (crypto_data);
						spin_lock_irqsave(&sc->
								  crypt_st_lock,
								  flags);
						removed_skb_num =
						    _remove_sta_skb_from_q(sc,
									   &sc->
									   preprocess_q,
									   sta_addr0_3,
									   sta_addr4_5);
	...
	if (vif_priv) {
		printk
		    ("vif: hw_en:%d, sw_en:%d, hw_de:%d, sw_de:%d, use_mac80211 %d, valid:%d\n",
		     (vif_priv->has_hw_encrypt == true),
		     (vif_priv->need_sw_encrypt == true),
		     (vif_priv->has_hw_decrypt == true),
		     (vif_priv->need_sw_decrypt == true),
...
static void _ssv6xxx_tx(struct ieee80211_hw *hw, struct sk_buff *skb)
{
	...
	struct ssv_sta_priv_data *ssv_sta_priv =
	    sta ? (struct ssv_sta_priv_data *)sta->drv_priv : NULL;
	struct ssv6200_tx_desc *tx_desc;
	int ret;
	unsigned long flags;
	bool send_hci = false;
	...
	do {
		...
		if (sc->dbg_tx_frame) {
			printk
			    ("================================================\n");
			_ssv6xxx_hexdump("TX frame", (const u8 *)skb->data,
					 skb->len);
		}
		if ((skb_info->crypt_st == PKT_CRYPT_ST_NOT_SUPPORT) &&
		    ieee80211_has_protected(hdr->frame_control)
		    && (ieee80211_is_data_qos(hdr->frame_control)
			|| ieee80211_is_data(hdr->frame_control))) {
			bool unicast = !is_broadcast_ether_addr(hdr->addr1);
			if ((unicast && (ssv_sta_priv != NULL)
...
int ssv6xxx_encrypt_task(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (skb_queue_len_safe(&sc->preprocess_q) == 0
		    || (ta->cpu_offline != 0)) {
			ta->running = 0;
				...
				if (enc_ret == 0) {
					...
					ori_prio };
						spin_lock_irqsave(&sc->
								  ...
								  flags);
						dev_err(sc->dev,
							"Set crypto task %d priority to %d.\n",
							this_task->pid,
							sp.sched_priority);
						sched_setscheduler(this_task,
								   this_task->
								   policy, &sp);
						cur_prio = ori_prio;
						spin_unlock_irqrestore(&sc->
...
#ifdef CONFIG_SMP
				dev_err(sc->dev,
					"%d-%d(%d) crypto fail, skb = %p %d %d\n",
					this_task->on_cpu, this_task->pid,
					cur_prio, skb, cont_crypto_failure,
					ta->paused);
#endif
				spin_unlock_irqrestore(&sc->crypt_st_lock,
						       flags);
				ssv_skb_free(skb);
				skb = NULL;
				if (cont_crypto_failure ==
				    max_cont_crypto_failure) {
					spin_lock_irqsave(&sc->crypt_st_lock,
							  flags);
			...
			if (ta->running != 0) {
				...
				if (ta->cpu_no == cpu_id) {
					...
					spin_lock_irqsave(&sc->crypt_st_lock,
							  flags);
					dev_warn(sc->dev,
						 "Crypto running on the same core. (%d, %d)\n",
						 cpu_id, num_running);
					spin_unlock_irqrestore(&sc->
							       crypt_st_lock,
							       flags);
...
void _clean_up_crypto_skb(struct ssv_softc *sc, struct ieee80211_sta *sta)
{
	...
	dev_dbg(sc->dev, "Removed %u MPDU from precess queue.\n",
		removed_skb_num);
	removed_skb_num =
	    _remove_sta_skb_from_q(sc, &sc->crypted_q, sta_addr0_3,
				   sta_addr4_5);
				     ...
				     u32 changed)
{
	...
	if (!priv_vif->vif_idx) {
		if (changed & BSS_CHANGED_BSSID) {
			...
			printk
			    ("BSS_CHANGED_BSSID: %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	if (vif->type == NL80211_IFTYPE_AP) {
		...
		if (changed & BSS_CHANGED_BEACON_INT) {
			printk
			    ("[A] BSS_CHANGED_BEACON_INT beacon_interval(%d)\n",
			     info->beacon_int);
			   ...
			   struct ieee80211_vif *vif, struct ieee80211_sta *sta)
{
		...
		do {
			...
			for (s = 0; s < SSV_NUM_STA; s++, sta_info++) {
				...
				if ((sta_info->s_flags & STA_FLAG_VALID)) {
					if (sta_info->sta == sta) {
						printk
						    ("search stat %02x:%02x:%02x:%02x:%02x:%02x to  wsid=%d\n",
	...
	do {
			...
			if ((sta_info->s_flags & STA_FLAG_VALID) == 0) {
				...
				if ((vif_priv->pair_cipher == SSV_CIPHER_WEP40)
					...
					if (vif_priv->crypto_data.ops != NULL) {
						sta_priv_dat->crypto_data.ops =
						    vif_priv->crypto_data.ops;
						sta_priv_dat->crypto_data.priv =
						    vif_priv->crypto_data.priv;
					}
		...
		printk
		    ("Add %02x:%02x:%02x:%02x:%02x:%02x to VIF %d sw_idx=%d, wsid=%d\n",
				...
				struct ieee80211_ampdu_params *params)
{
	...
	case IEEE80211_AMPDU_RX_START:
		...
		printk(KERN_ERR
		       "IEEE80211_AMPDU_RX_START %02X:%02X:%02X:%02X:%02X:%02X %d.\n",
		       sta->addr[0], sta->addr[1], sta->addr[2], sta->addr[3],
	...
	case IEEE80211_AMPDU_TX_START:
		printk(KERN_ERR
		       "AMPDU_TX_START %02X:%02X:%02X:%02X:%02X:%02X %d.\n",
		       sta->addr[0], sta->addr[1], sta->addr[2], sta->addr[3],
		       sta->addr[4], sta->addr[5], tid);
	...
	case IEEE80211_AMPDU_TX_OPERATIONAL:
		printk(KERN_ERR
		       "AMPDU_TX_OPERATIONAL %02X:%02X:%02X:%02X:%02X:%02X %d.\n",
		       sta->addr[0], sta->addr[1], sta->addr[2], sta->addr[3],
		       sta->addr[4], sta->addr[5], tid);
...
int ssv6xxx_skb_pre_encrypt(struct sk_buff *mpdu, struct ssv_softc *sc)
{
	...
	if ((crypto_data->ops != NULL)
	    && (crypto_data->ops->encrypt_prepare != NULL)) {
		u32 hdrlen = ieee80211_hdrlen(hdr->frame_control);
		ret =
		    crypto_data->ops->encrypt_prepare(mpdu, hdrlen,
						      crypto_data->priv);
	} else {
		ret = -EOPNOTSUPP;
	}
...
int ssv6xxx_skb_pre_decrypt(struct sk_buff *mpdu, struct ieee80211_sta *sta,
			    struct ssv_softc *sc)
{
	struct ssv_crypto_data *crypto_data = NULL;
	struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)mpdu->data;
	...
	START_READ_CRYPTO_DATA(crypto_data);
	if ((crypto_data->ops != NULL)
	    && (crypto_data->ops->decrypt_prepare != NULL)) {
		u32 hdrlen = ieee80211_hdrlen(hdr->frame_control);
		ret =
		    crypto_data->ops->decrypt_prepare(mpdu, hdrlen,
						      crypto_data->priv);
	} else {
		ret = -EOPNOTSUPP;
	}
...
int ssv6xxx_skb_encrypt(struct sk_buff *mpdu, struct ssv_softc *sc)
{
	...
	crypto_data = ssv6xxx_skb_get_tx_cryptops(mpdu);
	START_READ_CRYPTO_DATA(crypto_data);
	if ((crypto_data == NULL) || (crypto_data->ops == NULL)
	    || (crypto_data->priv == NULL)) {
		ret = -1;
	} else {
		u32 hdrlen = ieee80211_hdrlen(hdr->frame_control);
		ret =
		    crypto_data->ops->encrypt_mpdu(mpdu, hdrlen,
						   crypto_data->priv);
	}
	END_READ_CRYPTO_DATA(crypto_data);
	return ret;
						    ...
						    struct sk_buff *mpdu)
{
	...
	sta_priv = (struct ssv_sta_priv_data *)sta->drv_priv;
	vif_priv =
	    (struct ssv_vif_priv_data *)sta_priv->sta_info->vif->drv_priv;
	...
	if ((sta->drv_priv != NULL) && (vif_info->if_type == NL80211_IFTYPE_AP)) {
		...
		printk
		    ("[Local Crypto]: No useful drv_priv, sta = %p, unicast = %d, vif_priv = %p",
		     sta, unicast, vif_priv);
		if (sta != NULL)
			...
			struct ssv_softc *sc)
{
	...
	if (crypto_data != NULL) {
		...
		if ((crypto_data->ops != NULL) && (crypto_data->priv != NULL))
			ret =
			    crypto_data->ops->decrypt_mpdu(mpdu, hdrlen,
							   crypto_data->priv);
		END_READ_CRYPTO_DATA(crypto_data);
		return ret;
	}
...
extern void ssv6xxx_send_deauth_toself(struct ssv_softc *sc, const u8 * bssid,
				       const u8 * self_addr);
...
//    printk("mactime=%u, len=%d\r\n", *((u32 *)&rx_skb->data[28]), rxdesc->len);    //+++
	...
	if (rxdesc->rate_idx < SSV62XX_G_RATE_INDEX && rxphypad->RSVD == 0) {
		if (is_beacon || is_probe_resp) {
			...
			if (sta) {
				...
				if (is_beacon) {
					mgmt =
					    (struct ieee80211_mgmt *)(rx_skb->
								      data +
								      SSV6XXX_RX_DESC_LEN);
					p = cfg80211_find_ie(WLAN_EID_DS_PARAMS,
							     mgmt->u.beacon.
							     variable,
							     rx_skb->len);
					if (p) {
						...
						if (beacon_channel !=
							...
							if (diff_channel_cnt >=
								...
								printk
								    (KERN_DEBUG
								     "ssv6xxx_send_deauth_toself by channel change\n");
	...
	if ((rx_hw_dec == false) && (do_sw_dec == true)) {
		...
		if (ret == 0) {
			...
			if (sc->max_preprocess_q_len <
			    skb_queue_len(&sc->preprocess_q))
				sc->max_preprocess_q_len =
				    skb_queue_len(&sc->preprocess_q);
			spin_unlock_irqrestore(&sc->crypt_st_lock, flags);
			list_for_each_entry_reverse(ta, &sc->encrypt_task_head,
						    list) {
				if ((cpu_online(ta->cpu_no))
				    && (ta->running == 0)) {
					wake_up(&ta->encrypt_wait_q);
					return;
				}
			...
			if (ret < 0) {
				dev_err(sc->dev,
					"[Local Crypto]: Fail to decrypt local: %02X:%02X:%02X:%02X:%02X:%02X, ret = %d.\n",
					hdr->addr2[0], hdr->addr2[1],
					hdr->addr2[2], hdr->addr2[3],
					hdr->addr2[4], hdr->addr2[5], ret);
				goto drop_rx;
			}
		} else {
			...
			if (h_evt->h_event == SOC_EVT_NO_BA) {
				...
				if (h_evt->evt_seq_no == 0) {
					...
					if (time_after
						...
						printk
						    ("data[%ld] SDIO RX throughput %ld Kbps\n",
						     sc->sdio_rx_evt_size,
						     (sc->
						      sdio_rx_evt_size << 3) /
				     ...
				     ssize_t length)
{
	...
	buf_size -= prt_size;
	prt_size =
	    snprintf(status_buf, buf_size, "\tCrypto pre-process queue: %d.\n",
		     skb_queue_len(&sc->preprocess_q));
	status_buf += prt_size;
	buf_size -= prt_size;
	prt_size =
	    snprintf(status_buf, buf_size, "\tMax pre-process queue: %d.\n",
		     sc->max_preprocess_q_len);
	status_buf += prt_size;
	buf_size -= prt_size;
	prt_size =
	    snprintf(status_buf, buf_size, "\tCrypto process queue: %d.\n",
		     skb_queue_len(&sc->crypted_q));
	status_buf += prt_size;
	buf_size -= prt_size;
